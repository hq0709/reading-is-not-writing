...
# figure widths of the journal layout (inches); the paper size system: every figure is authored at the ICLR text
# width (5.5 in) and one of three fixed heights, and is included at width=\textwidth so nothing is rescaled
DOUBLE, SINGLE = 7.2, 3.5
WIDTH = 5.5
H1, H2, H3 = 2.4, 4.4, 6.2      # one-row, two-row, three-row figures
...
def save(fig, base: Path, dpi: int = 200):
    """PDF at the authored size (no tight cropping, so every figure keeps its exact width/height); PNG preview."""
