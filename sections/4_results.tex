...
\subsection{Every cell establishes robust linear availability}
...
\subsection{Matched generic controls reveal a causal split}
...
The clinical comparison changes the scientific meaning of that generic pass. Nodule changes mean $P(yes)$ by 0.2519 and realizes the maximum unrelated-direction effect. The reported Effusion-minus-maximum-unrelated margin is $-0.0573$. When the maximum over all five fixed clinical directions is recomputed in each bootstrap replicate, its two-sided 95\% CI is $[-0.0694,-0.0450]$ and its one-sided 95\% lower bound is $-0.0678$. The replicated effect is causally non-generic, but the named Effusion direction does not own it under the registered specificity criterion.

\subsection{The evidence ladder localizes each supported interpretation}

The results form one ordered finding. First, every target is robustly available to the same-capacity linear readout. Second, matched generic controls distinguish the two LLaVA cells from Qwen Effusion, so availability alone does not determine intervention selectivity. Third, Qwen's replicated generic-control pass meets a fixed family of clinical alternatives and yields a stronger Nodule response, separating non-generic causal influence from clinical identity. The shared location, dose, rows, controls, and uncertainty calculation make each rung an explicit claim boundary rather than an informal interpretation.
