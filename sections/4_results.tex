...
\subsection{Answer mapping separates response structure from clinical attribution}
...
Clinical competition adds a different observation. Table~\ref{tab:encoding-mass} compares the Mass direction with its strongest of five clinical competitors in finding-present logit coordinates at $\alpha=+0.25$. A-present assigns A to presence and B to absence; B-present reverses those assignments. The intervals are pointwise patient-bootstrap summaries.

\input{tables/table_encoding_mass}

Under both coded prompts, the Mass effect exceeds every other clinical effect, the largest of 20 random effects, and the absolute sham effect. Its clinical margin is negative under the standard prompt and positive under both coded mappings. The standard-prompt margin is recomputed on the 200 intervention patients, rather than the full 400-patient diagnostic cohort. This discovery was selected from six questions on reused patients; the coded prompts change wording and instructions together with the answer symbols. It identifies a prompt-conditioned candidate for independent confirmation, while the original fixed-prompt ownership verdict remains intact. The relevant causal comparison is thus indexed by the question and its prompt interface as well as the direction.

\subsection{Each control changes the scientific claim}

Controlled decoding tests label availability, non-semantic perturbations test write efficacy, and clinical competitors test attribution within a specified response interface. The original Qwen Effusion studies separate efficacy from attribution, while the crossover shows why the prompt interface belongs in that comparison. Table~\ref{tab:gates} collects the original intervention outcomes. The new Mass candidate changes the next empirical question, not the inferential status of the original six gates.
